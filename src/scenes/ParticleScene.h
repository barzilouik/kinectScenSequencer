...
class ParticleScene : public ofxFadeScene {
    float localScaledVol;
    
	public:

		// set the scene name through the base class initializer
		ParticleScene(float _localScaledVol) : ofxFadeScene("Particles") {
			particles.setAutoRemove(false);	// don't remove particles if dead
			setSingleSetup(false); // call setup each time the scene is loaded
			setFade(1000, 1000); // 1 second fade in/out
            
            localScaledVol = _localScaledVol;
		}
		...
		void draw() {
            ...
            glPointSize(2);
			ofSetColor(255, 255, 255, 128*alpha);
		...
		static const int s_maxNumParticles = 3000;
};
